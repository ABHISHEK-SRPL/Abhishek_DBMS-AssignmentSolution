...
CREATE TABLE PASSENGER ( Passenger_name VARCHAR(100), Category VARCHAR(100), Gender VARCHAR(10), 
Boarding_City VARCHAR(100), Destination_City VARCHAR(100), Distance INT, Bus_Type VARCHAR(100));
...
SELECT Passenger_name, P.Boarding_City, P.Destination_city, P.Bus_Type, PR.Price FROM passenger P, price PR WHERE 
...
-- 8. What will be the Sitting and Sleeper bus charge for Pallavi to travel from Bangalore to Panaji?
   ...
   SELECT DISTINCT P.PASSENGER_NAME, P.Boarding_city AS Destination_city, P.DESTINATION_CITY AS BOARDING_CITY, P.Bus_Type, 
   PR.PRICE FROM PASSENGER P, PRICE PR WHERE PASSENGER_NAME = 'PALLAVI' AND P.DISTANCE = PR.DISTANCE;
...
-- distance travelled by all passengers without using user variables (NEED TO CHECK THIS ONE)
...
SELECT DISTINCT Price.Distance, Price, CASE WHEN Price > 1000 THEN 'EXPENSIVE' WHEN Price > 500 AND Price < 1000 THEN 
'AVERAGE COST' ELSE 'CHEAP' END AS VERDICT FROM Price INNER JOIN Passenger ON Price.Distance = Passenger.Distance;